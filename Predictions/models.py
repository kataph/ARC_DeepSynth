...
class BigramsPredictor(nn.Module):
    ...
    def __init__(self,
        ...
        H = IOEncoder.output_dimension * self.IOEmbedder.output_dimension
        # TODO: FC: why guessing? if latent encoder is not compatible with H an error will raise
        # And if it is compatible the output size will always be equal to latent_encoder(smthng).size()[0] 
        # of course, latent encoder is supplied during initialization, thus one may not know immediately
        # such a value, but it could be passed as an additional argument.
        output_size = guess_output_size(latent_encoder, H)
