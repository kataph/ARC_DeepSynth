        ...
        while len(list_to_be_treated) > 0:
            ...
            if depth == max_program_depth - 1:
                ...
                if current_type.hash in self.hashed_type_expansions:
                    ...
                    for P,args in P2arguments_dict.items():
                        decorated_arguments_P=[encode_non_terminal(arg, get_new_context(context,P,i), depth+1) for i,arg in enumerate(args)]
                        rules[non_terminal].update({P:decorated_arguments_P})
                        list_to_be_treated.extendleft((arg, get_new_context(context,P,i), depth + 1) for i,arg in enumerate(args) if (arg, get_new_context(context,P,i), depth + 1) not in list_to_be_treated)
