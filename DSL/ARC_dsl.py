...
# overrides frozenset, tuple, union
# overrides ALL!!!
...
#     b: FrozenSet
# ) -> FrozenSet:
#     """ returns the intersection of two FrozenSet """
#     return a & b
def intersection(
    a: FrozenSet[t0],
    b: FrozenSet[t0]
...
#TODO: keep only the one with frozen set?
def merge_cf(
    containers: FrozenSet[FrozenSet[t0]]
) -> FrozenSet[t0]:
    """ merging """
    return type(containers)(e for c in containers for e in c)
def merge_ct(
...
def valmax_cf(
    container: FrozenSet[t0],
    compfunc: Arrow[t0,Integer] #This should be Arrow(t0,INT)
) -> Integer:
    """ maximum by custom function """
    return compfunc(max(container, key=compfunc, default=0))
def valmax_ct(
...
def valmin_cf(
    container: FrozenSet[t0], 
    compfunc: Arrow[t0,Integer] #idem as above
) -> Integer:
    """ minimum by custom function """
    return compfunc(min(container, key=compfunc, default=0))
def valmin_ct(
...
# def mfilter(
#     container: Container, #TODO: error
#     function: Callable
# ) -> FrozenSet:  #TODO: error?
#     """ filter and merge """
...
def product_cf( #technically the second t0 should be t1 but better to reduce
    a: FrozenSet[t0],
    b: FrozenSet[t0]
) -> FrozenSet[Couple[t0,t0]]: # TODO: this is the only function with a polymorphic couple...
    """ cartesian product """
    return frozenset((i, j) for j in b for i in a)
def product_ct( #technically the second t0 should be t1 but better to reduce
    a: Tuple[t0],
    b: Tuple[t0]
) -> Tuple[Couple[t0,t0]]:
    """ cartesian product """
...
#     condition: Boolean,
#     a: Any, #TODO need a polym. type that can become arrows, anything
#     b: Any
# ) -> Any:
#     """ if else branching """
...
# def branch( #error in solutions with branch taking callables and obtaining identities? -> NO ERROR! if condition set identity else do something
#     condition: Boolean,
#     a: a0, #TODO need a polym. type that can become arrows, anything. But maybe only built types?; It is however rare that a0 is a non-primitive callable (found one anonymous forked function) 
#     b: a0 #I assume homogeneity
# ) -> a0:
#     """ if else branching """
#     return a if condition else b
def branch( #error in solutions with branch taking callables and obtaining identities? -> NO ERROR! if condition set identity else do something
...
def fork( #assume hom. -> cannot do, there is fork(recolor, ...)
    outer: Arrow[t1, Arrow[t2,t3]],
    a: Arrow[t0,t1],
    b: Arrow[t0,t2]
) -> Arrow[t0,t3]:
    """ creates a wrapper function """
...
def rapply( #I do only tuple for this one and assume hom. funcs. TODO: it may not work
    functions: Tuple[Arrow[t0,t1]], # TODO: this is painful, only Tuple[Arrow]
    value: t0
) -> Tuple[t1]:
    """ apply each function in container to value """
    return type(functions)(function(value) for function in functions)
...
def mapply_cf(
    function: Arrow[FrozenSet[t0],FrozenSet[t0]],
    container: FrozenSet[FrozenSet[t0]]
) -> FrozenSet[t0]: #error?
    """ apply and merge """
    return merge(apply(function, container))
...
def mpapply(
    function: Arrow[t0,Arrow[t1,Tuple[t2]]],
    a: Tuple[t0],
    b: Tuple[t1]
) -> Tuple[t2]:
    """ apply function on two vectors and merge """
    return merge_ct(papply(function, a, b))
