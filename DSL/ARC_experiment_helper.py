...
def make_program_checker(dsl: ARC_DSL, examples) -> Callable[[Program, bool], bool]:
...
def make_program_checker_with_constants(dsl: ARC_DSL, examples, constants) -> Callable[[Program, bool], Tuple[bool, Program]]:
...
def task_set2dataset(tasks, model, dsl: ARC_DSL) -> List[Tuple[str, ARC_PCFG, Callable[[Program, bool], bool]]]:
...
def task_set2uniform_dataset(tasks, dsl: ARC_DSL, max_program_depth: int = 4) -> List[Tuple[str, ARC_PCFG, Callable[[Program, bool], bool]]]:
    dataset = []
    # Prepare batch
    for task in tasks:
        if len(task) == 3:
        ...
        grammar = dsl.ARC_DSL_to_ARC_CFG(type_req, max_program_depth=max_program_depth).CFG_to_Uniform_PCFG()
        dataset.append(
            (name, grammar, make_program_checker_with_constants(dsl, examples, constants) if constants else make_program_checker(dsl, examples)))
    return dataset
