...
def __get_type(el, fallback=None):
    if isinstance(el, bool):
        ...
        if len(el) > 0:
            #FC: it is assuming (correctly) that lists have homogeneous type
...
def filter_tasks_for_model(tasks, model) -> typing.List[Tuple[str, Any]]:
    '''FC: checks that (i) if model is RulesPredictor the IOs are list int > list int and
                       (ii) the IOs satisfy some basic consistency rules defined in filter_examples'''
    filtered_tasks = []
    for task in tasks:
        name, examples = task
